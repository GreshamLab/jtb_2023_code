...
from jtb_2022_code.utils.figure_data import CommonNames
...
def predict_all(
    ...
    count_model = read(SUPIRFACTOR_COUNT_MODEL).eval().set_time_parameters(
        n_additional_predictions=0,
        loss_offset=0
    )
    velo_model = read(SUPIRFACTOR_VELOCITY_DYNAMICAL_MODEL).eval().set_time_parameters(
        n_additional_predictions=0,
        loss_offset=0
    )
    biophysics_model = read(SUPIRFACTOR_BIOPHYSICAL_MODEL).eval().set_time_parameters(
        n_additional_predictions=0,
        loss_offset=0
    )
    ...
    if velo_model._count_inverse_scaler is None:
        velo_model.set_scaling(
            _count_scaler,
            _velo_scaler
        )
        
    if biophysics_model._decay_model._count_inverse_scaler is None:
        biophysics_model._decay_model.set_scaling(
            _count_scaler,
            _velo_scaler
        )
...
def _add_predict_times(
    ...
    predicts.obs['program_rapa_time'] = np.tile(
        predict_times,
        int(predicts.shape[0] / predict_times.shape[0])
    )
...
def process_data_for_model(data, genes=None, wt_only=True, scale=True):
    ...
    if genes is not None and ((len(genes) != model_data.shape[1]) or not all(model_data.var_names == genes)):
...
def process_velocity_for_model(data_obj, genes=None, scale=True):
    ...
    for i in range(1, 3):
        ...
        _velo.append(dd.layers['rapamycin_velocity'] + dd.layers['cell_cycle_velocity'])
...
def predict_from_model(
    ...
    **kwargs
    ...
    if return_data_stacked:
        ...
        predicts_training = np.concatenate(
            predicts_training,
            3
        )
...
def load_model_input_data():
    ...
    adata.obs['Test'] = _get_test_idx(adata.shape[0])

    adata.X = adata.X.astype(np.float32)
    sc.pp.normalize_per_cell(adata, min_counts=0)

    adata.layers['scaled'] = count_scaling.fit_transform(
        adata.X
    )
    ...
    return adata, count_scaling, velo_scaling
