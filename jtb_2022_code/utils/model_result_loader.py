...
_max_n = 12
cat_colors = [
    colors.rgb2hex(plt.get_cmap("plasma", _max_n)(c))
    for c in range(_max_n)
]

rng = np.random.default_rng(100)
rng.shuffle(cat_colors)

metric_label = {"AUPR": "AUPR", "R2_validation": "R${}^2$"}


def load_model_results(modeling_type=None, trim_nans=True):
    supirfactor_results = pd.read_csv(
        MODEL_RESULTS_FILE, sep="\t",
        low_memory=False
    )
    _fix_loaded(supirfactor_results)

    if modeling_type is not None:
        supirfactor_results = supirfactor_results[
            supirfactor_results[modeling_type]
        ].copy()

    if trim_nans:
        supirfactor_results = supirfactor_results.dropna(
            subset="Learning_Rate"
        )
        _to_cats(supirfactor_results)

    supirfactor_losses = pd.read_csv(
        MODEL_LOSSES_FILE,
        sep="\t",
        low_memory=False
    )
    _fix_loaded(supirfactor_losses)

    if modeling_type is not None:
        supirfactor_losses = supirfactor_losses[
            supirfactor_losses[modeling_type]
        ].copy()

    if trim_nans:
        supirfactor_losses = supirfactor_losses.dropna(
            subset="Learning_Rate"
        )
        _to_cats(supirfactor_losses)
...
def _fix_loaded(df):
    ...
    df["Shuffle"] = df["Shuffle"].astype(str)
...
def _to_cats(df):
    ...
    df["Learning_Rate"] = df[
        ...
        pd.CategoricalDtype(
            np.sort(df["Learning_Rate"].unique()),
            ordered=True
        )
    )
    df["Weight_Decay"] = df[
        ...
        pd.CategoricalDtype(
            np.sort(df["Weight_Decay"].unique()),
            ordered=True
        )
    )
    df["Decay_Model_Width"] = df[
        "Decay_Model_Width"
    ].astype(
        pd.CategoricalDtype(
            np.sort(df["Decay_Model_Width"].unique()),
            ordered=True
        )
    )
...
def summarize_model_results(model_results, modeling_type="Counts"):
    _summary_idx = (model_results["Learning_Rate"] == 5e-5) & (
        model_results["Weight_Decay"] == 1e-7
    )
    _summary_idx |= pd.isna(model_results["Learning_Rate"])
    _summary_idx &= (model_results["Shuffle"] == "False") | (
        model_results["Shuffle"] == "Prior"
    )
    ...
    summary_results.loc[
        summary_results["Model_Type"] == "inferelator", "model"
    ] = "inferelator"
    ...
    summary_results = summary_results.dropna(subset="model").drop_duplicates(
        subset=["model", "Seed", "prior_shuffle"]
    )

    summary_results["model"] = summary_results["model"].astype(
        pd.CategoricalDtype(
            [
                "inferelator",
                "static_meta",
    ...
    summary_results["x_loc"] = (
        summary_results["model"].cat.codes * 4
        + summary_results["prior_shuffle"].astype(int)
        + 0.5
    )
    summary_results["x_color"] = summary_results["model"].cat.codes.map(
        lambda x: colors.rgb2hex(matplotlib.colormaps["Dark2"](x))
    )
    summary_results.loc[
        summary_results["prior_shuffle"],
        "x_color"
    ] = "#000000"

    model_stats = summary_results.groupby(
            ["x_loc", "model", "prior_shuffle", "x_color"]
        )["AUPR"].agg(
            ["std", "mean"]
        ).dropna(
        ).reset_index()
...
def plot_results(
    ...
    plot_data["x_color"] = plot_data[param].cat.codes.map(
        lambda x: cat_colors[x]
    )

    plot_stats = (
        plot_data.groupby(["x_loc", "x_color"])[metric]
        .agg(["std", "mean"])
        .dropna()
        .reset_index()
    )
