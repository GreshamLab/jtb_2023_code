...
def get_elife_model_predictions(
    ...
    with torch.no_grad():
        predictions = {
            ...
            for k, v in TF_LOOKUP.items() if (
                v is None or
                v in biophysical_model.prior_network_labels[1]
            )
        }
    ...
    return ypd, rapa, scaler, predictions, tfa_predictions, prediction_gradients
...
def get_elife_data(
    ...
    if genes is not None and ((len(genes) != data.shape[1]) or not all(data.var_names == genes)):
        data = data[:, genes].copy()
    
    if rapa:
        data.obs['program_rapa_time'] = np.random.default_rng(100).uniform(27.5, 32.5, size=data.shape[0])
    else:
        data.obs['program_rapa_time'] = np.random.default_rng(100).uniform(-10, 0, size=data.shape[0])
...
def get_elife_predicts(elife, model, genotype="WT", scaler=None):
