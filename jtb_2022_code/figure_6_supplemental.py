...
def figure_6_supplement_1_plot(
    predictions,
    rapa,
    save=True
):
    ...
    for i, _g in enumerate(_gene):
        ...
        for j, (_tf, _tf_common, _tf_name) in enumerate(
            ...
            if j == 0:
                axd[i, j].set_title(chr(65 + i), loc='left', size=8, weight='bold', x=-0.28)
                axd[i, j].set_ylabel("Expression\n(Counts)", size=8)

    if save:
        fig.savefig(FIGURE_6_SUPPLEMENTAL_FILE_NAME + "_1.png", facecolor='white')
...
def figure_6_supplement_2_plot(
    ...
    _plot_tfa_difference_and_marginal_loss(
        ...
        ['YER040W', 'YEL009C', 'YJL110C', 'YOL067C', 'YBL103C', 'YDR463W', 'YHR006W', 'YIR023W'],
        prediction_gradients,
        axd['tfa_all'],
        axd['loss_all'],
        _has_tfa,
    ...
    if save:
        fig.savefig(FIGURE_6_SUPPLEMENTAL_FILE_NAME + "_2.png", facecolor='white')
