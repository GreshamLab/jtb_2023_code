...
def _do_cellrank(data, npcs, nns):
    ...
    if "X_pca" in data.obsm and data.obsm["X_pca"].shape[1] >= npcs:
        ...
        _sc.pp.pca(data, n_comps=npcs, use_highly_variable=False, zero_center=False)
...
def cellrank_grid_search(adata, layer="counts"):
    ...
    if CELLRANK_OBSM_COL not in adata.obsm:
        adata.obsm[CELLRANK_OBSM_COL] = _pd.DataFrame(_np.zeros((adata.shape[0],
                                                                 len(OBSM_COLUMNS))),
    ...
    for npc in N_PCS:
        for nn in N_NEIGHBORS:
            ...
            cellrank_pt = _cellrank_by_group(adata, layer=layer, npc=npc, nns=nn)
