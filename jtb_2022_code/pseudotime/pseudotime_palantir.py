...
from ..utils.projection_common import (
    do_pc1_min_cell
)
...
def _do_palantir(data, npcs, ncomps=15, nns=30):
    ...
    with _parallel_backend("loky", inner_max_num_threads=1):
        ...
        _pca_df = _pd.DataFrame(
            data.obsm["X_pca"][:, 0:npcs],
            index=data.obs_names
        )
        dm_res = palantir.utils.run_diffusion_maps(
            _pca_df, n_components=ncomps, knn=nns
        )

        print("\tMultiscale Space")
        ms_data = palantir.utils.determine_multiscale_space(dm_res)
        _ = harmony.plot.force_directed_layout(
            dm_res["kernel"],
            data.obs_names
        )

        # Get pseudotimes based on PCA
        start_cell = do_pc1_min_cell(data)

        print(f"\tPalantir (Start Cell: {start_cell})")
        pr_res = palantir.core.run_palantir(
            ms_data, start_cell, num_waypoints=500, knn=nns
        )
...
def palantir_grid_search(
    ...
    if PALANTIR_OBSM_COL not in adata.obsm:
        adata.obsm[PALANTIR_OBSM_COL] = _pd.DataFrame(
    ...
    for npc in n_pcs:
        for nn in N_NEIGHBORS:
            ...
            palantir_pt = _palantir_by_group(
                adata,
                layer=layer,
                npc=npc,
                nns=nn,
