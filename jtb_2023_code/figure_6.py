...
def _gradient_to_marginal(grad_tuple):
    return torch.multiply(grad_tuple[1], grad_tuple[2]).numpy()


def _plot_elife_predicts(
    predictions, rapa, gene, genotype, genotype_common, ax, seed=44
):
    ...
    _rapa = rapa.X[_rapa_tf_idx, _gene_idx]
    ...
    ax.plot([25, 35], [_rapa.mean()] * 2, color="black")
    ax.plot(
        [25, 35],
...
def _plot_tfa_difference_and_marginal_loss(
    ...
    for _i, tf in enumerate(tfs):
        ...
        _tfa = np.hstack(
            (
                prediction_gradients[None][2][:, -1, include_tf_index]
                .mean(axis=0)
                .reshape(-1, 1),
                prediction_gradients[tf][2][:, -1, include_tf_index]
                .mean(axis=0)
                .reshape(-1, 1),
            )
        ).T
        ...
        if tfa_difference_ax is not None:
            tfa_difference_ax.plot(x_axis, _tfa, color="black", alpha=0.1)
            ...
            tfa_difference_ax.plot(
                x_axis, _tfa[:, plot_tfs.isin(_ups)], color="red", alpha=1
            )
        ...
        if marginal_loss_ax is not None:
            ...
            for _i, _t in enumerate(_ups):
                ...
                marginal_loss_ax.annotate(
                    ...
                    (
                        x_axis[1] + 0.05,
                        _marginal_loss[-1, plot_tfs == _t] - _offset
                    ),
                    color="red",
                    size=7,
                )
    ...
    if tfa_difference_ax is not None:
        ...
        _labs = []
        for t in tfs:
            _common_t = common_name(t).lower()
            _labs.append("WT")
            _labs.append(f"Δ{_common_t}")

        tfa_difference_ax.tick_params("x", length=0, which="major")
        tfa_difference_ax.set_xticks(
            np.arange(0, x_all.max(), 2), _labs, rotation=90, size=8
        )
    ...
    if marginal_loss_ax is not None:
        ...
        marginal_loss_ax.tick_params(
            "x", length=0, which="major", pad=marginal_labelpad
        )
...
def plot_figure_6(
    ...
    axd["tfa"].set_title(
        "A", size=8, loc="left", weight="bold", y=1.05, x=-0.1
    )
    axd["predict_2"].set_title(
        "B", size=8, loc="left", weight="bold", y=1.15, x=-0.1
    )
    axd["gln3_tfa"].set_title(
        "C", size=8, loc="left", weight="bold", y=1.05, x=-0.1
    )
    axd["marginal_loss"].set_title(
        "D", size=8, loc="left", weight="bold", y=1.05, x=-0.1
    )
    ...
    axd["marginal_loss"].annotate(
        "TFA Over-\nestimated", (0.05, 0.875), xycoords="axes fraction", size=8
    )
    axd["marginal_loss"].annotate(
        "TFA Under-\nestimated", (0.05, 0.2), xycoords="axes fraction", size=8
    )
