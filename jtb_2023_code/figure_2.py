...
from inferelator_velocity.utils.keys import PROGRAM_KEY
...
def plot_figure_2(data, save=True):
    ...
    axd = {
        ...
        "pseudotime_pca": fig.add_axes(
            [
                _pt_hist_left,
                _bottom_y + _bottom_h / 3 * 2,
                _pt_hist_width,
                _bottom_h / 3,
            ]
        ),
        "pseudotime_dpt": fig.add_axes(
            [
                _pt_hist_left,
                _bottom_y + _bottom_h / 3,
                _pt_hist_width,
                _bottom_h / 3
            ]
        ),
        "pseudotime_palantir": fig.add_axes(
            [
                _pt_hist_left,
                _bottom_y,
                _pt_hist_width,
    ...
    _ami_linkage = linkage(
        squareform(
            data.all_data.uns[PROGRAM_KEY][f"{_metric}_distance"],
            checks=False
        )
    )
    _ami_dendrogram = dendrogram(_ami_linkage, no_plot=True)
    _ami_idx = np.array(_ami_dendrogram["leaves"])
    _ami_distances = data.all_data.uns[PROGRAM_KEY][f"{_metric}_distance"][
        _ami_idx, :
    ][
        :, _ami_idx
    ]
    ...
    _prog_series = (
        data.all_data.var.loc[_gene_order, PROGRAM_KEY].astype(int)
        + _cat_series.max()
        + 1
    )
    ...
    axd["rep_cc"].set_title("Cell Cycle Phase", size=8)
    axd["rep_cc"].set_xlim(-10, 60)
    ...
    axd["method_hm"].set_yticks(
        range(hm_data.shape[0]),
        labels=hm_data.index.map(lambda x: hm_labels[x]),
        size=8,
    )
    ...
    axd["legend_rapa"].imshow(
        plt.imread(FIG_RAPA_LEGEND_VERTICAL_FILE_NAME), aspect="equal"
    )
    axd["legend_rapa"].axis("off")

    axd["legend_cc"].imshow(
        plt.imread(FIG_CC_LEGEND_VERTICAL_FILE_NAME), aspect="equal"
    )
...
def _get_pt_hist(data_df, bins=80, key="time_pca_pt"):
    ...
    return [
        np.bincount(
            pd.cut(
                data_df.loc[data_df["Pool"] == x, key],
                cuts,
...
def _make_method_heatmap_data(pdata):
    return (
        pdata.max_pseudotime_rho.loc[(slice(None), "WT"), (slice(None), False)]
        .melt(ignore_index=False)
        .reset_index()
        .drop(["Gene", "denoised"], axis=1)
        .pivot_table(index="method", columns="Experiment", values="value")
    )
