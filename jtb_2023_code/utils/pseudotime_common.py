...
def spearman_rho_pools(
    pool_vector, pt_vector, average_1_2_pools=True, ignore_na_inf=False
):
    if (
        not ignore_na_inf and
        (any(_pd.isna(pt_vector)) or any(_np.isinf(pt_vector)))
    ):
        return _np.nan
